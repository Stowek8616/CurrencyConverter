...
public class CurrencyConverterTest extends JFrame {
    ...
    private static final String API_URL = "https://manage.exchangeratesapi.io/dashboard" + API_KEY + "/latest/USD";
    ...
    private static void DisplayImage() {
        try {
            ...
            // Create the original image display
            ImageIcon originalIcon = new ImageIcon(img);
            JLabel originalLabel = new JLabel(originalIcon);
            
            // Create the resized image (10x10 is very small, consider larger values)
            BufferedImage resizedImage = resizeImage(img, 100, 100);
            ImageIcon resizedIcon = new ImageIcon(resizedImage);
            JLabel resizedLabel = new JLabel(resizedIcon);
            
            // Create a panel to display both images
            JPanel panel = new JPanel();
            panel.setLayout(new BorderLayout());
            panel.add(resizedLabel, BorderLayout.SOUTH);
